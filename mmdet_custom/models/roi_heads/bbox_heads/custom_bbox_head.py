...
class Custom_BBoxHead(BaseModule):
    ...
    def __init__(self,
                 ...
                 init_cfg=None):
        super(Custom_BBoxHead, self).__init__(init_cfg)
        ...
        self.proj_loss_bbox = proj_loss_bbox
    ...
    def _get_target_single(self, pos_bboxes, neg_bboxes, pos_gt_bboxes,
        ...
        bbox_targets_proj = pos_bboxes.new_zeros(num_samples, 4)
        ...
        if num_pos > 0:
            ...
            if self.proj_loss_bbox:
                pos_bbox_targets_proj = pos_gt_bboxes
                bbox_targets_proj[:num_pos, :] = pos_bbox_targets_proj
            bbox_targets[:num_pos, :] = pos_bbox_targets
            bbox_weights[:num_pos, :] = 1
        if num_neg > 0:
            label_weights[-num_neg:] = 1.0
        if self.proj_loss_bbox:
            return labels, label_weights, (bbox_targets, bbox_targets_proj), bbox_weights
        else:
    ...
    def get_targets(self,
        ...
        labels, label_weights, bbox_targets, bbox_weights = multi_apply(
            ...
            cfg=rcnn_train_cfg)
        if self.proj_loss_bbox:
            bbox_targets, bbox_targets_proj = list(zip(*bbox_targets))
            if concat:
                labels = torch.cat(labels, 0)
                label_weights = torch.cat(label_weights, 0)
                bbox_targets = torch.cat(bbox_targets, 0)
                bbox_targets_proj = torch.cat(bbox_targets_proj, 0)
    ...
    def loss(self,
        ...
        if bbox_pred is not None:
            if self.proj_loss_bbox:
                bbox_targets, bbox_targets_proj = bbox_targets
            ...
            if pos_inds.any():
                ...
                if self.proj_loss_bbox:
                    bbox_pred_proj = self.bbox_coder.decode(rois[:, 1:], bbox_pred)
                    if self.reg_class_agnostic:
                        pos_bbox_pred_proj = bbox_pred_proj.view(
                            bbox_pred_proj.size(0), 4)[pos_inds.type(torch.bool)]
                    else:
                        pos_bbox_pred_proj = bbox_pred_proj.view(
                            bbox_pred_proj.size(0), -1,
                ...
                if self.proj_loss_bbox:
                    losses['loss_bbox'] = self.loss_bbox(
                        pos_bbox_pred,
                        pos_bbox_pred_proj * (1/scale_factor),
                        bbox_targets[pos_inds.type(torch.bool)],
                        bbox_targets_proj[pos_inds.type(torch.bool)] * (1/scale_factor),
